...
At the start of each test block we fetch 4 things:
...
Also as part of this we consider whether or not we need to replace certificates in the updater binary, resulting in output like this:
...
.. code-block:: text
    ...
    Scanning log for failures and warnings
    --------------------------------------
    TEST-UNEXPECTED-FAIL: update status was not successful: 
    check_updates returned failure for Linux_x86-gcc3 downloads/firefox-135.0b9.tar.xz vs. downloads/firefox-136.0b2.tar.xz: 1
    TEST-UNEXPECTED-FAIL: update status was not successful: 
    check_updates returned failure for Linux_x86-gcc3 downloads/firefox-135.0b9.tar.xz vs. downloads/firefox-136.0b2.tar.xz: 1
    -------------------------
    This run has failed, see the above log

The square-bracketed string (eg: ``[135.0 bn complete]``) can be searched for in the log to find the full output from the test that failed.
